...
public class PreferenceServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req,
        ...
        String[] titleStyleAndWeight = req
                .getParameterValues("titleStyleAndWeight");
        String titleFontSize =
                req.getParameter("titleFontSize");
        resp.addCookie(new Cookie("maxRecords", maxRecords)
        );
        resp.addCookie(new Cookie("titleFontSize",
                titleFontSize));
        // 先删除titleFontWeight 和 titleFontStyle cookies
        // 通过设置maxAge = 0
        ...
        if (titleStyleAndWeight != null) {
            for (String style : titleStyleAndWeight) {
                if (style.equals("bold")) {
                    ...
                    resp.addCookie(new Cookie("titleFontStyle",
                            "italic"));
                }
            }
        }
        resp.setContentType("text/html");
        PrintWriter writer = resp.getWriter();
        writer.println("<html><head>" + "<title>Preference</tit le>"
                        + "</head><body>" + MENU
